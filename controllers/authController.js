...
// Onlt for renderred page, no error!
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError(
          `You dont have enough permission to perform this operation`,
          401,
        ),
      );
    }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: 'Your password reset token valid within 10mins',
      message,
    });
    ...
    return next(
      new AppError('There was an error sending the email. Pls try again later'),
      500,
    );
  }
});
